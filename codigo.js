...
var fila="<tr id='border'><td class='id'></td><td class='foto'></td><td class='price'></td><td class='title'></td><td id='hola' class='description'></td><td class='category'></td><td class='eliminar'></td></tr>";
	 var productos=null;
  function codigoCat(catstr) {
	...
	function listarProductos(productos) {
	  ...
	  var borra=document.getElementById("price"); 
	  precio.setAttribute("onclick", "orden*=-1;listarProductos(productos);");
...
function obtenerProductos() {
	  fetch('https://retoolapi.dev/39uiuF/productos')
            .then(res=>res.json())
            .then(data=>{
				productos=data;
				productos.forEach(
				...
				productos=data;listarProductos(data)})
...
function borrarProductos() {
	...
	const $id2 = document.getElementById('jo')
	
	id2=document.getElementById('idBorrar.value')
	fetch('https://retoolapi.dev/39uiuF/productos/'+id2,{ method:"DELETE"})
	.then(res=>res.json())
